...
function processAjaxResponse(jqXHR, data) {
  if (jqXHR.getResponseHeader("X-Login-Page")) {
    ...
    if (
      (refreshStop !== undefined && refreshStop.toString() === "1") ||
      (dataCurrentContest !== undefined &&
        newCurrentContest !== dataCurrentContest.toString())
    ) {
      window.location.reload();
      return;
    }
...
document
  .querySelectorAll(".desktop-scoreboard .forceWidth:not(.toolong)")
  .forEach((el) => {
...
function resizeMobileTeamNamesAndProblemBadges() {
  ...
  document
    .querySelectorAll(".mobile-scoreboard .forceWidth:not(.toolong)")
    .forEach((el) => {
  ...
  document
    .querySelectorAll(".mobile-scoreboard .mobile-problem-badges:not(.toolong)")
    .forEach((el) => {
      el.classList.remove("toolong");
